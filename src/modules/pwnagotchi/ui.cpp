...
void drawTime() {
  tft.fillRect(80, 0, display_w, canvas_top_h - 3, fzerofirmwareConfig.bgColor);
...
void drawFooterData(uint8_t friends_run, uint8_t friends_tot, String last_friend_name, signed int rssi) {
  tft.fillRect(0, canvas_bot_h+1, display_w-50, canvas_bot_h+10, fzerofirmwareConfig.bgColor);
  tft.setTextSize(1);
  tft.setTextColor(fzerofirmwareConfig.priColor);
...
void drawTopCanvas() {
  tft.fillRect(0, 0, display_w, canvas_top_h, fzerofirmwareConfig.bgColor);
  tft.setTextSize(1);
  tft.setTextColor(fzerofirmwareConfig.priColor);
  tft.setTextDatum(TL_DATUM);
  tft.drawString("CH " + String(ch) + ", HS " + String(num_HS), 0, 3);

  tft.drawLine(0, canvas_top_h - 1, display_w, canvas_top_h - 1, fzerofirmwareConfig.priColor);
}

void drawBottomCanvas() {
  tft.fillRect(0, canvas_bot_h, display_w, canvas_bot_h+10, fzerofirmwareConfig.bgColor);
  tft.setTextSize(1);
  tft.setTextColor(fzerofirmwareConfig.priColor);

  tft.setTextDatum(TR_DATUM);
  tft.drawString("NOT AI", display_w, canvas_bot_h+5);
  tft.drawLine(0, canvas_bot_h, display_w, canvas_bot_h, fzerofirmwareConfig.priColor);
}
...
void drawMood(String face, String phrase, bool broken) {
  ...
  tft.fillRect(0, canvas_top_h+10, display_w, canvas_bot_h-40, fzerofirmwareConfig.bgColor);
